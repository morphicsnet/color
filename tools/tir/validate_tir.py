...
Validate T-IR JSON documents against docs/ir/tir-schema.json using jsonschema.

Usage:
  python tools/tir/validate_tir.py [--schema docs/ir/tir-schema.json] [--examples] [files...]
  python tools/tir/validate_tir.py --stdin
...
def main(argv: List[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Validate T-IR JSON files against the T-IR schema")
    ap.add_argument("--schema", default="docs/ir/tir-schema.json", help="Path to tir-schema.json")
    ap.add_argument("--stdin", action="store_true", help="Read a single JSON document from stdin")
    ap.add_argument("--examples", action="store_true", help="Validate all files under docs/ir/examples/")
    ap.add_argument("--verbose", action="store_true", help="Verbose output for valid files")
    ap.add_argument("files", nargs="*", help="Paths to T-IR JSON files")
    ...
    if any_fail:
        return 1
    print("[ok] All T-IR documents valid")
    return 0
